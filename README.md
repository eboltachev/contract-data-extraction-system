...
- `backend/`: Python 3.12 + FastAPI + Clean Architecture/DDD слои.
...
Критерии читаются динамически из листа шаблона с заголовками `№`, `Критерий`, `Значение`. Для каждого критерия строится план, выбираются релевантные фрагменты договора, затем значения извлекаются конкурентно с ограничением `MAX_CONCURRENT_LLM_REQUESTS`. Лимит агентных итераций задает `AGENT_MAX_ITERATIONS`.
